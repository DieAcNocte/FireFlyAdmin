/**
 * frontmatter 解析与序列化。
 * 解析用 gray-matter（与博客生态一致）；序列化自己实现，
 * 以匹配项目的书写风格：YAML 日期保持裸格式（js-yaml 会解析为 Date，
 * 加引号会导致 Astro content schema 的 z.date() 校验失败）。
 */
...
export function parseFrontmatter(raw: string): { data: Record<string, unknown>; body: string } {
	...
	return { data: (parsed.data ?? {}) as Record<string, unknown>, body };
}
